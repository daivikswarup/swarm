...
//RETURNS 0 IF POSITIVE ELSE RETURNS 1
...
//RETURNS CORRESPONDING ANGLE BETWEEN -PI AND +PI
double normalizeAngle(double x)
{
	return x-2*M_PI*floor((x+M_PI)/(2*M_PI));
}

class path_parameters//REQUIRED FOR DEFINING A PATH
{
	public:
	double k1;
	double k2;
...
class polar//HAS FUNCTIONS TO CONVERT CARTESIAN TO POLAR AND TO CALULATE V AND W
{
	public:
	double ro;
	double gamma;
